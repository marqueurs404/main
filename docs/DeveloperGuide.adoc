...
image::nusmods/EventClassDiagram.png[align="center", width=75%, scaledwidth=18cm]
...
We have implemented an `Api` component to contain the logic of interfacing with external APIs.
...
The following class diagram shows the expected internal structure of the API component:

.Expected implementation of API Component in `v2.0` to improve abstraction. Currently there is no `ApiManager` which manages external interactions with other components, other components are accessing the `xxxApi` classes and `Cache` class directly for accessing API data.
